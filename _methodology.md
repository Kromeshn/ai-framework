...
| Файл | Содержит | Форма | Когда обновляется |
|---|---|---|---|
| `B0-context.md` | Бизнес-контекст: цель, домен (ключевые сущности и правила), известные ограничения | Бизнес без реализации: ни функций, ни кода | При смене бизнес-целей или scope |
| `L1-architecture.md` | Архитектура системы: компоненты, внешние системы, потоки данных | Диаграммы композиции и потоков + таблицы эндпоинтов и зависимостей | При структурных изменениях (добавлен компонент, поменялась интеграция) |
| `L2-functions.md` | Функции скрипта: входы, выходы, ветвления | Схема на функцию; текст — подпись (ограничение / «почему» / подводный камень), не пересказ; контракты — таблицами | После каждого изменения кода, затрагивающего контракт или логику функции |
| `ROADMAP.md` | Шаги проекта: что делаем, выход, зависимости, статусы | Таблица шагов: что делаем / выход / статус ✅ | При дискавери и пересмотре пути; ✅ шагу при закрытии цикла |
| `_backlog.md` | Парковка отложенного вне активного scope: риски/дефекты и идеи, у каждого пункта условие возврата | Таблица: строка на пункт, суть + условие возврата | Пункт добавляется при осознанном откладывании; уходит в работу, когда условие возврата наступило |
