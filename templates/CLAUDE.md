...
Предложения и объяснения в чат — бизнес-языком, через поведение системы.

---

## Правила работы с артефактами

- Безличность: в документации не писать от первого лица. Не «мы отдаём `0`», а «сервис отдаёт `0`» либо «возвращается `0`».
- Без шрамов сессии: долгоживущие файлы (docs, планы, ROADMAP) не содержат следов диалога — жаргона сессии, мета-комментариев о процессе, оправданий правок. Тест перед записью: текст понятен читателю, не видевшему чата.
- Плотность: документация — полевая шпаргалка, только смысл. Вводка 1–2 строки, ячейки таблиц телеграфно, каждый факт в файле один раз.
- Правки по команде: любые правки любых файлов — только по явной команде-императиву («правь», «пиши», «делай»). Вопрос, «будем», цитата пожелания «надо сделать X» — обсуждение, ответ в чат. Команда на этап покрывает все правки внутри этапа.
