...
- Правки по команде: вопрос или напоминание — не команда редактировать. Находка → рассказ в чате → явное добро → правка. Файлы в `docs/` создаются и правятся только по явному указанию; без него draft даётся в чат.
